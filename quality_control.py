...
def main():
	...
	(total_reads_raw_fastq, base_count_raw, mean_raw_read_length) = count_bases(sys.argv[3])
	(total_reads_trimmed_fastq, base_count_trimmed, mean_trimmed_read_length) = count_bases(sys.argv[4])
	(count_mRNA, percent_mRNA, percent_tRNA, percent_rRNA) = count_rna_type(sys.argv[5],reads_mapped)	 
	(percent_ncRNA) = count_nc_rna(sys.argv[5], reads_mapped)
	##make output file

	percent_base_remaining = float(base_count_trimmed)/base_count_raw*100

	m = re.match(r'^(MURI_\d+_[AGCT]+_\w+)_\w+',sys.argv[1])	
	if m:
		sample = m.group(1)
	...
	outFile.write("sample\ttime\ttotal_read_count\ttotal_base_count\tmean_raw_read_length\ttrimmed_read_count\tpercent_reads_remaining\ttrimmed_base_count\tpercent_base_remaining\tmean_trimmed_read_length\tmapped_read_count\tpercent_trimmed_reads_mapped\tcount_mapping_mRNA\tpercent_mapping_mRNA\tpercent_mapping_tRNA\tpercent_mapping_to_rRNA\tpercent_mapping_to_ncRNA\n")
	row = "%s\ttime\t%i\t%i\t%i\t%i\t%.4g\t%i\t%.4g\t%i\t%i\t%.4g\t%i\t%.4g\t%.4g\t%.4g\t%.4g\n" % (sample_name, total_reads_flexbar, base_count_raw, mean_raw_read_length, reads_trimmed, percent_reads_remaining, base_count_trimmed, percent_base_remaining, mean_trimmed_read_length, reads_mapped, percent_mapped_reads, count_mRNA, percent_mRNA, percent_tRNA, percent_rRNA, percent_ncRNA )
	outFile.write(row)
	outFile.close()
